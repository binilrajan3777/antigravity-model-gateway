      ...
      if (item.parts) {
        ...
        if (hasFunctionCall && item.role === 'model') {
          ...
          if (role === 'assistant') {
            ...
            for (const p of parts) {
              if (p.text) {
                ...
                if (id.mimeType && id.mimeType.startsWith('image/')) {
                  log.info(`[Proxy] image part -> ${id.mimeType}, base64 ${(id.data || '').length} chars`);
                  imageParts.push({
                    type: 'image_url',
                    image_url: { url: `data:${id.mimeType};base64,${id.data}` },
                  });
                } else {
            ...
            const textContent = partsContent.join('\n');
            // Images must travel as their own content parts. A data: URI pasted into
            // the text body arrives as literal characters - which is why the model
            // replies that it cannot see the image.
            if (imageParts.length > 0) {
              const multimodal: OpenAIContentPart[] = [];
              if (textContent) multimodal.push({ type: 'text', text: textContent });
